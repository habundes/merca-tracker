...
- Crear árbol `app/` (route files finos re-exportan screens existentes; imports relativos — no hay alias `@/`):
  ...
  - `app/(tabs)/_layout.tsx` — `<Tabs>` con 3 tabs, iconos Ionicons y estilos actuales; `initialRouteName: 'search'` vía `unstable_settings`.
  - Tab **Search**: `app/(tabs)/search.tsx` (re-export `SearchScreen`), `headerShown:true` a nivel tab (título "Search").
  - Tab **List**: `app/(tabs)/list/_layout.tsx` (`<Stack>`), `index.tsx`→ListMain ("Mis búsquedas"), `config.tsx`→ListConfig ("Configurar list"), `[itemId].tsx`→ItemDetail ("Detalle").
  - Tab **Profile**: `app/(tabs)/profile/_layout.tsx` (`<Stack>`), `index.tsx`→ProfileMain ("Profile"), `account.tsx`→AccountSettings ("Ajustes de cuenta"), `payment.tsx`→PaymentSettings ("Ajustes de pago").
...
- Auth gating / rutas protegidas / redirects. El login sigue viviendo dentro de la screen `profile/index` (= ProfileMain), con estado local efímero como hoy.
...
|---|---|---|
| `list/.../ListMain.tsx` | `navigation.navigate('ListConfig')` | `const router = useRouter(); router.push('/list/config')` |
| `list/.../ListMain.tsx` | `navigation.navigate('ItemDetail', { itemId: item })` | `router.push(\`/list/${encodeURIComponent(item)}\`)` |
| `list/.../ListConfig.tsx` | `navigation.navigate('ItemDetail', { itemId: 'demo-1' })` | `router.push('/list/demo-1')` |
| `list/.../ItemDetail.tsx` | `route.params.itemId` (prop) | `const { itemId } = useLocalSearchParams<{ itemId: string }>()` |
| `profile/.../ProfileMain.tsx` | `navigation.navigate('AccountSettings')` / `'PaymentSettings'` | `router.push('/profile/account')` / `router.push('/profile/payment')` |
...
- Único payload de params en toda la app: **`itemId: string`** → ruta dinámica `app/(tabs)/list/[itemId].tsx`, leído con `useLocalSearchParams`.
...
- Tabs: `screenOptions` global `headerShown:false`; Search sobreescribe `headerShown:true`. Iconos por `name` de tab (mismos glyphs Ionicons) y estilos de `tabBar` idénticos a `AppTabs.tsx`.
...
5. **Tabs layout.** `app/(tabs)/_layout.tsx`: `<Tabs>` replicando `AppTabs.tsx` (iconos Ionicons `List/search/Profile`→`list/search/profile`, colores/estilos de `tabBar`, `initialRouteName:'search'` vía `unstable_settings`). Definir las 3 `<Tabs.Screen>` con su `title`/`tabBarIcon`; `search` con `headerShown:true`.
6. **Tab Search.** `app/(tabs)/search.tsx` re-exporta `SearchScreen`. Verificar que la tab renderiza y `useSearch()` funciona.
7. **Stack List.** `app/(tabs)/list/_layout.tsx` (`<Stack>` con titles). `index.tsx`→ListMain, `config.tsx`→ListConfig, `[itemId].tsx`→ItemDetail. Reescribir navegación de `ListMain` y `ListConfig` a `useRouter().push(...)`; `ItemDetail` a `useLocalSearchParams`.
8. **Stack Profile.** `app/(tabs)/profile/_layout.tsx` (`<Stack>` con titles). `index.tsx`→ProfileMain, `account.tsx`→AccountSettings, `payment.tsx`→PaymentSettings. Reescribir `ProfileMain` a `useRouter().push('/profile/account' | '/profile/payment')`; quitar props de `AccountSettings`/`PaymentSettings`.
...
10. **Verificación final.** `npx tsc --noEmit` limpio. Arrancar app: tab inicial Search; `list`→config→[itemId] y back; `profile`→account/payment y back; login efímero de ProfileMain intacto; header de Search visible; glass y `useSearch()` operativos.
...
- [x] Existe `app/_layout.tsx` (SearchProvider + StatusBar + Stack `headerShown:false`) y `app/(tabs)/_layout.tsx` (Tabs, `initialRouteName:'search'`, iconos/estilos conservados).
- [x] Rutas: `(tabs)/search`, `(tabs)/list/{index,config,[itemId]}`, `(tabs)/profile/{index,account,payment}`; cada route file re-exporta la screen de `src/features/*` (imports relativos).
- [x] Títulos idénticos: List "Mis búsquedas"/"Configurar list"/"Detalle"; Profile "Profile"/"Ajustes de cuenta"/"Ajustes de pago"; Search con header a nivel tab.
...
- [x] En la app: tab inicial Search; navegar `list`→config→detalle de un ítem (con `itemId` correcto) y volver; `profile`→account/payment y volver; login/logout efímero de ProfileMain igual; glass y `useSearch()` operativos.
...
- **Paridad 1:1, sin deep linking real ni auth gating** (decisión explícita del usuario): `scheme` se añade solo porque Expo Router lo requiere; redirects/rutas protegidas y linking público son otro spec. Login sigue efímero dentro de `profile/index`.
...
| R2 | `initialRouteName:'search'` no aplica: Expo Router enfoca la primera ruta por orden de archivo, no por `initialRouteName` directo | Usar `export const unstable_settings = { initialRouteName: 'search' }` en `(tabs)/_layout.tsx`; verificar tab inicial en criterio de aceptación. |
...
| R4 | Header duplicado en Search (header de tab + header de algún stack) o ausente | Tabs `screenOptions.headerShown:false` global; `search` fija `headerShown:true`; los stacks List/Profile manejan su propio header. El criterio verifica el header de Search. |
