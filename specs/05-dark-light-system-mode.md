...
| R10 | En iOS 26+, el pill del botón "atrás" del header nativo (chrome real de Liquid Glass gestionado por UIKit/`react-native-screens`, no por `expo-glass-effect`) se ve claro un instante durante la transición de push antes de adoptar el tinte correcto. Verificado en simulador iPhone 17 Pro tras rebuild nativo (`npx expo run:ios`) — persiste. | **Aceptado como fuera de scope**, igual que el resto del header real de Liquid Glass (ver "No incluye" y R7). No existe prop JS equivalente a `colorScheme` (de `expo-glass-effect`) para forzar el tinte de este chrome nativo. Investigar en spec futuro si existe workaround (p.ej. `headerTransparent`, header custom sin glass, o si Apple expone una API nueva en SDKs posteriores). |
